...
- 自动生成数据质量治理快照（before/after）及问题明细。
...
#### 模块0：数据质量治理（`/quality`、`/quality_effect`）

- 最新批次质量总览（治理前后指标对比）；
- 问题率分布（缺失/重复/薪资解析失败/学历非法/异常值）；
- 多批次治理成效趋势（当前默认展示最近5个批次）；
- 批次明细表支持报告截图与答辩展示。
...
- `GET /api/quality/compare`：治理成效对比（支持 `limit`，默认5）
...
```mermaid
flowchart LR
    A[原始数据源<br/>51job.xlsx] --> B[数据导入模块<br/>data_loader.py]
    B --> C[字段映射与基础解析<br/>薪资解析 parse_salary]
    C --> D[质量评估 Before<br/>missing/duplicate/edu/salary/outlier]
    D --> E[治理规则执行<br/>标准化+去重+异常处理]
    E --> F[质量评估 After]

    D --> G[(data_quality_snapshot)]
    F --> G
    D --> H[(data_quality_issues)]

    E --> I[(jobs_51job 原始业务表)]
    I --> J[分析模块<br/>analysis1~4]
    J --> K[(分析结果缓存表<br/>CitySalaryDistribution等)]

    G --> L[质量总览页面<br/>/quality]
    G --> M[治理成效页面<br/>/quality_effect]
    H --> L
    I --> N[业务分析页面<br/>/analysis1~4]
    K --> N
```

说明：
- `batch_id` 作为每次导入的治理快照主键，串联 `before/after` 与问题明细；
- 页面展示分为两条线：**治理线**（`/quality`、`/quality_effect`）和**业务分析线**（`/analysis1~4`）。
...
- 建议补充 `.env` 配置加载、单元测试/接口测试、Docker 一键部署。
